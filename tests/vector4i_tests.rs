...
fn clamp_method() {
    ...
    assert_eq!(
        Vector4i::new(-5, 5, 15, <int!()>::MAX).clamp(&Vector4i::default(), &vector),
        Vector4i::new(0, 5, 10, 10),
        "clamp should work as expected."
    );
    assert_eq!(
        vector.clamp(
            &Vector4i::new(0, 10, 15, -10),
            &Vector4i::new(5, 10, 20, -5)
        ),
        Vector4i::new(5, 10, 15, -5),
        "clamp should work as expected."
    );
}
...
fn operators() {
    ...
    assert_eq!(
        Vector4i::from(Vector4::new(1.1, 2.9, 3.9, 100.5)),
        Vector4i::new(1, 2, 3, 100),
